...
from database import (
    ...
    get_alerts, get_categories, get_companies,
...
if not df_latest.empty:
    ...
    for col, (_, row) in zip(cols, df_latest.iterrows()):
        # หาราคาก่อนหน้า
        hist_cat = df[df["category"] == row["category"]]["price"]
...
with tab1:
    fig = px.line(
        dff, x="recorded_at", y="price", color="category",
        title="แนวโน้มราคาเศษเหล็กตามหมวด",
        labels={"recorded_at": "วันที่", "price": "ราคา (฿/กก.)", "category": "หมวด"},
        markers=True,
    )
...
with tab2:
    ...
    if not df_latest.empty:
        df_bar = df_latest[df_latest["category"].isin(sel_cats)] if sel_cats else df_latest
        fig2 = px.bar(
            df_bar.sort_values("price", ascending=True),
            x="price", y="category", orientation="h",
            color="price", color_continuous_scale="RdYlGn_r",
            title="ราคาล่าสุด — เปรียบเทียบทุกหมวด",
            labels={"price": "ราคา (฿/กก.)", "category": "หมวด"},
            text="price",
        )
    ...
    fig3 = px.box(
        dff, x="category", y="price", color="category",
...
with tab3:
    if sel_cats:
        ...
        df_ohlc = dff[dff["category"] == cat_ohlc].set_index("recorded_at")["price"]
...
with tab4:
    show_df = dff[["recorded_at", "company", "price_date", "category", "price", "sender"]].copy()
    show_df.columns = ["บันทึกเมื่อ", "บริษัท", "วันที่ในข้อความ", "หมวด", "ราคา (฿/กก.)", "ผู้ส่ง"]
